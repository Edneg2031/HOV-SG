...
若楼层、房间和物体均已成功，但导航阶段报错：

```text
ValueError: Found array with 0 sample(s) ... required by DBSCAN
```

说明固定 `1.5m` 相机高度推算出的地面位置略低于稀疏点云观测到的楼层下界，导致该楼层
没有分配到相机 pose。当前 `navigation_graph.py` 已增加回退：严格估算未匹配 pose 时，
按相机中心是否处于楼层垂直范围进行归属。同步该文件后重新执行第 5 节命令。
